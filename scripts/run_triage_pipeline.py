...
def cmd_pilot_or_full(args) -> None:
        ...
        for ev in group:
            ...
            if routes_to_l3:
                ...
                record = staging.build_record({
                    ...
                    "model": args.model, "prompt_version": layer2.PROMPT_VERSION,
            ...
            pilot_rows.append({
                ...
                "verdict": answer.is_adc, "confidence": "text" if not answer.from_recall else "recall",
                "quote": answer.quote, "from_recall": answer.from_recall, "routed_to_layer3": routes_to_l3,
            })
    ...
    # ---- Layer 3 (pilot mode: skipped by default — Layer 2 alone is the pilot's point) ----
    if args.mode == "full":
        ...
        for c in within_cap:
            try:
                answers, log = layer3.run_layer3([{"program_id": c["program_id"], "name": c["name"]}], model=args.model)
            except model_client.ModelClientError as e:
                _log_failure(c["program_id"], c["name"], e)
                n_failed += 1
                continue
            answer = answers[c["program_id"]]
            try:
                ...
                continue
            record = staging.build_record({
                "program_id": c["program_id"], "proposed_name": c["name"], "is_adc": answer.is_adc,
                "layer": 3, "model": args.model, "prompt_version": layer3.PROMPT_VERSION,
                "quote": answer.quote,
            }, run_id=run_id)
            staging.append_record(record)
            n_staged += 1

        # ---- validation draw (Step C's last part) ----
        staged = staging.load_records()
        this_run = [r for r in staged if r.get("run_id") == run_id and not r.get("manual_overflow")]
        sample = tval.draw_stratified_sample(this_run)
    ...
    if args.mode == "pilot":
        ...
        out = REPORTS_DIR / "triage_pilot.md"
        lines = [
            "# Triage pilot report", "",
            f"Run `{run_id}` — {len(pilot_rows)} candidate(s), real Layer 2 spend ${total_cost:.4f}.", "",
        ...
        for r in pilot_rows:
            quote = (r["quote"] or "").replace("|", "\\|").replace("\n", " ")
            lines.append(
                f"| {r['name']} | {r['verdict']} | {r['confidence']} | {quote} | "
                f"{r['from_recall']} | {r['routed_to_layer3']} |"
            )
        out.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print(f"\nWrote {out}. Stop here and review it before spending anything more.")
...
def main() -> None:
    ...
    modes = [m for m in (args.dry_run, args.pilot, args.full) if m]
    if len(modes) != 1:
        print("Pass exactly one of --dry-run / --pilot / --full.")
        sys.exit(1)
