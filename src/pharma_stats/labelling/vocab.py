...
#            (with a reason) is terminal. "not_an_adc" lives on the reason
#            enum so an is_adc=no row can also carry a consistent
#            in_scope=no (the two fields are independent; a non-ADC is
#            out of this project's filter). Gate 2 is never reached unless
#            is_adc is already yes, so a reviewer cannot pick not_an_adc
#            there — that would contradict the molecule judgement.
...
# not_an_adc is only written with is_adc=no (gate 1). The other four are
# the gate-2 reasons for a real ADC that's out of this project's filter.
SCOPE_OUT_REASONS = ["heme_only", "pre_2012", "non_industry", "non_oncology", "not_an_adc"]
GATE2_SCOPE_OUT_REASONS = ["heme_only", "pre_2012", "non_industry", "non_oncology"]
GATE_VALUES = [1, 2, 3]
